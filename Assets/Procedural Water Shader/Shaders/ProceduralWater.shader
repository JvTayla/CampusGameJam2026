    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            Varyings vert(Attributes v)
            {
                ...
                float3 worldPos0 = TransformObjectToWorld(v.positionOS.xyz);
                ...
                float3 displacedOS = v.positionOS.xyz + TransformWorldToObjectDir(offset);
            ...
            half4 frag(Varyings i) : SV_Target
            {
                ...
                float3 refrRgbRaw = SampleSceneColor(screenUV);
                float3 refrRgb = refrRgbRaw * lerp(1.0, absorb, 0.75);
                ...
                float dC = sceneDepth;
                float dR = SampleSceneDepthLinear(screenUV + float2(px.x, 0.0));
                float dL = SampleSceneDepthLinear(screenUV + float2(-px.x, 0.0));
                float dU = SampleSceneDepthLinear(screenUV + float2(0.0, px.y));
                float dD = SampleSceneDepthLinear(screenUV + float2(0.0, -px.y));
                ...
                outCol.rgb = MixFog(outCol.rgb, i.fogCoord);
                return outCol;
            }
            ENDHLSL
        }
